...
from shared.ui_components import add_standard_header_controls, install_status_bar, mark_destructive_buttons
...
def _write_embedded_template(
    ...
    """Восстанавливает точный исходный .xlsx-файл из встроенных данных."""
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _refresh_action_icons(self) -> None:
        mappings = [
            ...
            ("btn_matrix_preview", "comparison.png"),
        ...
        for kind, state in getattr(self, "type_import_state", {}).items():
            buttons = ((state.get("download"), "free-icon-download-126488.png"),
                       ...
                       (state.get("preview"), "comparison.png"),
        ...
        approval_buttons = (
            ...
            (approval_state.get("preview"), "comparison.png"),
    ...
    def _setup_ui(self):
        ...
        existing_layout.addWidget(existing_hint); existing_layout.addStretch(1)
        self.project_mode_stack.addWidget(existing_page)
        create_layout.addWidget(self.project_mode_stack, 1)
        ...
        project_tab_layout.addWidget(create_card, 1)
    ...
    def _download_type_import_template(self, kind: str) -> None:
        try:
            ...
            if target:
                ...
                self._show_success("Готово", f"Шаблон сохранён: {target}")
                self._log(f"⬇️ Встроенный шаблон {kind} сохранён: {target}")
        except Exception as exc:
            self._show_alert("Ошибка", f"Не удалось сохранить шаблон:\n{exc}")
    ...
    def _setup_approval_tab(self) -> None:
        ...
        title = QtWidgets.QLabel("Маршруты согласований")
    ...
    def _download_approval_template(self) -> None:
        try:
            ...
            if target:
                ...
                shutil.copyfile(path, target)
                self._show_success("Готово", f"Шаблон сохранён: {target}")
                self._log(f"⬇️ Встроенный шаблон маршрутов сохранён: {target}")
        except Exception as exc:
            self._show_alert("Ошибка", f"Не удалось сохранить шаблон:\n{exc}")
    ...
    def _show_approval_details(self) -> None:
        ...
        title = QtWidgets.QLabel("Предпросмотр маршрутов согласований")
