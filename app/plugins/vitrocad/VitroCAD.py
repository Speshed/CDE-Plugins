...
class ToolShell(QtWidgets.QMainWindow):
    ...
    @staticmethod
    def _add_template_button(card: QtWidgets.QFrame, button: QtWidgets.QPushButton) -> None:
        """Place the template action beside the upload action in the file row."""
        button.setObjectName("downloadButton")
        button.setFixedHeight(34)
        button.setMinimumWidth(170)
        row_widget = card.findChild(QtWidgets.QWidget, "fileRowLast")
        if row_widget is None or row_widget.layout() is None:
            raise RuntimeError("Не найден ряд кнопок карточки файла")
    ...
    def _rebuild_folder_page(self) -> None:
        ...
        template_btn.clicked.connect(lambda: self._download_vitro_template("folders"))
    ...
    def _rebuild_permissions_page(self) -> None:
        ...
        template_btn = QtWidgets.QPushButton("Скачать шаблон")
        template_btn.clicked.connect(lambda: self._download_vitro_template("permissions"))
    ...
    def _rebuild_schedule_page(self) -> None:
        ...
        p.btn_excel.setText("Загрузить файл")
        template_btn = QtWidgets.QPushButton("Скачать шаблон")
        template_btn.clicked.connect(lambda: self._download_vitro_template("schedule"))
    ...
    def _download_vitro_template(self, kind: str) -> None:
        ...
        title, filename = defaults[kind]
        path, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, title, filename, "Excel (*.xlsx)"
        )
        if not path:
            return
        try:
            if not path.lower().endswith(".xlsx"):
                path += ".xlsx"
            write_vitrocad_template(kind, path)
            QtWidgets.QMessageBox.information(self, "Шаблон сохранён", f"Шаблон сохранён:\n{path}")
        except Exception as exc:
            QtWidgets.QMessageBox.critical(self, "Ошибка создания шаблона", str(exc))
