...
class ModernSection(QtWidgets.QWidget):
    ...
    def __init__(self, icon: str, title: str, collapsible: bool = True, parent=None):
        ...
        self.icon_label = QtWidgets.QLabel(self.header)
        self.icon_label.setObjectName("sectionIcon")
        if icon == "preview.png":
            runtime_root = Path(getattr(sys, "_MEIPASS", Path(__file__).resolve().parents[3]))
            self.icon_label.setPixmap(themed_icon(runtime_root / "assets", icon, False).pixmap(QtCore.QSize(18, 18)))
            self.icon_label.setProperty("iconAsset", icon)
            self.icon_label.setProperty("iconSize", 18)
        else:
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _setup_ui(self):
        ...
        sec_plan = ModernSection("preview.png", "Предпросмотр плана", collapsible=False)
