...
RETURN_TO_MANAGER_CODE = 75


def classify_plugin_exit(return_code: int | None) -> str:
    """Classify a child result independently from any UI text."""
    if return_code == RETURN_TO_MANAGER_CODE:
...
def shared_style_overrides(dark: bool) -> str:
    ...
    # Minimal overlay-style scrollbars: transparent track, rounded neutral thumb,
    # no arrow buttons. This matches the calm Desktop-manager visual language.
    scroll = "rgba(145,145,150,0.42)" if dark else "rgba(95,100,108,0.34)"
    scroll_hover = "rgba(180,180,186,0.62)" if dark else "rgba(70,75,84,0.52)"
    scroll_pressed = "rgba(205,205,210,0.72)" if dark else "rgba(55,60,68,0.66)"
    ...
    return f"""
        ...
        /* Login/connection actions are deliberately neutral by default. */
        QPushButton#loginButton, QPushButton#loginButton:default, QPushButton#loginButton:focus {{
            background: {bg}; color: {text}; border: 1px solid {border};
        }}
        QPushButton#loginButton:hover {{
            background: {hover}; color: {text}; border-color: {border};
        }}
        QPushButton#loginButton:pressed {{
            background: {pressed}; color: {text}; border-color: {border};
        }}
